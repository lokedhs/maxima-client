...
(defmacro dimension-bind-new ((stream output-record
                               &key
                                 ...
                                 ((:baseline baseline-sym)))
                              &body body)
  (alexandria:with-gensyms (width height tx ty x y transformation)
    (alexandria:once-only (stream output-record)
      (labels ((make-body ()
                 `(progn ,@body))
               (make-baseline ()
                 (if baseline-sym
                     ...
                     `(multiple-value-bind (,tx ,ty)
                        ...
                        ,(if no-transform
                             `(let ((,x ,tx)
                                ...
                                ,(make-coordinate-bind-form))
                             `(multiple-value-bind (,x ,y)
                                  (clim:untransform-position ,transformation ,tx ,ty)
                                (declare (ignorable ,x ,y))
                                ,(make-coordinate-bind-form))))
                     (make-baseline))))
...
(defun call-with-interactive-form (stream buttons-p builder-fn loop-fn)
      ...
      (let ((rec (clim:updating-output (stream)
                   (when active
                     ...
                     (when buttons-p
                       ...
                       (format stream "~%"))))))
        (unwind-protect
             (funcall loop-fn rec stream)
          (setq active nil)
          (clim:redisplay rec stream))))))

(defmacro with-interactive-form ((stream &key record-name buttons-p) builder &body body)
  (check-type stream symbol)
  (check-type record-name (or symbol null))
  (alexandria:with-gensyms (rec)
    (alexandria:once-only (buttons-p)
      `(call-with-interactive-form ,stream ,buttons-p
                                   (lambda () ,builder)
                                   (lambda (,rec ,stream)
                                     (declare (ignorable ,rec))
                                     (let (,@(if record-name `((,record-name ,rec))))
                                       ,@body))))))
